...
export async function POST(request: NextRequest) {
  const payload = await request.json().catch(() => null) as { name?: unknown; handle?: unknown; email?: unknown; password?: unknown; confirmPassword?: unknown } | null;
  ...
  if (!name || name.length > 60 || !/^[a-z0-9_]{3,20}$/.test(handle) || !/^\S+@\S+\.\S+$/.test(email) || password.length < 8 || password !== confirmPassword) return NextResponse.json({ error: "Use a name, a 3-20 character handle, a valid email, and matching 8+ character passwords." }, { status: 400 });
